...
const ALL_ITEMS = ALL_STATES;

const TOTAL = ALL_ITEMS.length; // 50
...
function loadSavedState(): StatesAllState {
  try {
...
function statesAllReducer(state: StatesAllState, action: StatesAllAction): StatesAllState {
  switch (action.type) {
    case 'GUESS_CORRECT': {
...
export function StatesAllScreen() {
  ...
  useEffect(() => {
    if (state.status !== 'WIN') return;
    const saved = localStorage.getItem('game_besttime_states-all');
        ...
        <form onSubmit={handleSubmit} className="input-group">
          <div className="input-wrapper">
            <input
              ...
              placeholder="Type any US state name"
